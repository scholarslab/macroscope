...
model = function (spec) {
    ...
    years = function () {
      // PMB necessary to handle dates < 1000 CE
        return this.yearly_total().keys().map(Number);
      //  return this.yearly_total().keys().map(Number);
    };
    that.years = years;
    ...
    set_meta = function (meta_s) {
        ...
        my.start_date = new Date(0, 0, 1); // nothing pre-Gutenberg in JSTOR
   ...
   // load topic_labels from a string of JSON
    set_topic_labels = function (topic_labels_s) {
      var parsed;

      if (typeof topic_labels_s !== 'string') {
        return;
      }

     parsed = JSON.parse(topic_labels_s);
     my.topic_labels = parsed;
      /*
       *   .topic_labels.map(function (topic) {
       *       var result = d3.map();
       *       topic.words.map(function (w, j) {
       *         result.set(w, topic.weights[j]);
       *       });
