...
//                .toUpperCase();
        return doc_author_string(m, i)[0];
        };
    }
...
doc_author_string = function (m, i) {
    ...
    if (doc.authors.length > 0) {
      result = doc.authors[0];
          } else if ($doc.authors.length > 1) {
       result = doc.authors.join(", ");
/*
...
doc_uri = function (m, d) {
    ...
    return "data/poems/" + m.meta(d).doi.replace(/\//, "_");
...
// PMB This is the per-topic yearly plot (in the list view and indiv topic view)
plot_topic_yearly = function (m, t, param) {
    ...
    // PMB modified to handle dates < 1000 CE
    var topic_years_list = m.topic_yearly(t).keys().map(Number).sort(function(a, b) {
        return a - b;
    });

    series = topic_years_list.map(function (y) {
...
doc_view = function (m, d) {
    ...
    view.select("#doc_remark")
        .html(m.dt.row_sum(doc) + " tokens matched in topic set. "
                + '<a class ="external" href="'
                + doc_uri(m, doc)
                + '.html">View poem '
                + m.meta(doc).doi.replace(/\//, "_")
                + "</a>");
...
model_view_list = function (m, sort, dir) {
    ...
    if (sort_choice === "words") {
        ...
        keys = keys.map(function (t) {
            ...
            // PMB Modified to handle dates < 1000 CE
            var topic_year_map = m.topic_yearly(t);
            var topic_years = topic_year_map.keys().map(Number).sort(function(a, b) {
                return a - b;
            });
//            m.topic_yearly(t).forEach(function (year, weight) {
            topic_years.forEach(function (year, index, array) {
              var weight = topic_year_map.get(year);
...
model_view_plot = function(m, type) {
    ...
    cloud_size = Math.floor(circle_radius * 2.4);
    ...
    gs.enter().append("g")
        .each(function (p, t) {
            ...
            g.append("circle")
                ...
                .attr("r", function (p) {
//                    return p.r;
                    return p.r * scale_radius(m.dt.col_sum(p.t));
                })
            ...
            tooltip().text(topic_label(m, t, 20));
            tooltip().update_pos();
            tooltip().show();
...
model_view_yearly = function (m, type) {
    ...
    paths.enter()
        ...
        .on("mouseover", function (d) {
            ...
            tooltip().text(topic_label(m, d.t, 10));
            tooltip().update_pos();
            tooltip().show();
        })
        .on("mousemove", function (d) {
...
// PMB This generates the big rainbow "Years" plot
yearly_stacked_series = function (m, raw) {
    ...
    if (!VIS.model_view.yearly.data) {
        ...
        // PMB modified to handle dates < 1000 CE
        year_keys = m.years().map(function(y) { return +y; }).sort(function(a, b) {
            return a - b;
        });
