...
//        minor: "alpha"
    ...
    overview_words: 19,     // may need adjustment
    model_view: {
...
//        size_range: [7, 18], // points. may need adjustment
        ...
        stroke_range: 8,    // max. perimeter thickness
/*        yearly: {
            w: 1090,
            h: 800,
            m: {
...
*/
    ...
    topic_view: {
        ...
        xticks: 20, // applied to x axis
        yticks: 10 // applied to y axis
    },
    word_view: {
        n_min: 19, // words per topic
...
topic_label = function (m, t, n) {
    ...
    if (m.topic_labels(String(t + 1)) !== undefined) {
      label += " (";
      label += m.topic_labels(String(t + 1));
      label += ")";
    }
...
/*
        ...
        // PMB Removed to handle Chinese names
    ...
    if (param.axes) {
        ...
        svg.append("g")
            ...
            .call(d3.svg.axis()
                .scale(scale_x)
                .orient("bottom")
                .ticks(d3.time.years, spec.xticks));
...
doc_view = function (m, d) {
    ...
    view.select("#doc_remark")
        .html(m.dt.row_sum(doc) + " tokens matched in topics. "
                + '<a class ="external" target="blank" href="'
                + doc_uri(m, doc)
                + '.html">View poem '
...
//tooltip().text(topic_label(m, d.t, 4));
            tooltip().text(topic_label(m, t, 19));
...
//    console.log("scale color is " + scale_color(d.t));
...
main = function () {
    load_data(dfb.files.info,function (error, info_s) {
        ...
        load_data(dfb.files.topic_labels, function (error, tl_s) {
            m.set_topic_labels(tl_s);
            view_refresh(m, window.location.hash);
        });
